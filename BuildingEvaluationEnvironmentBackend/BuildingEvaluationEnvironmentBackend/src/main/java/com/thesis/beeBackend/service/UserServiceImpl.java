...
public class UserServiceImpl implements UserService {
    ...
    public ResponseEntity<LoginResponseDTO> googleSignIn(UserDTO userDTO) {
        ...
        if (existingUser != null || message.equals("New user created")) {
            return ResponseEntity
                    .ok(new LoginResponseDTO(message, generateToken(userDTO.getEmail(), userDTO.getUsername())));
        } else {
    ...
    public ResponseEntity<LoginResponseDTO> login(UserDTO userDTO) {
        ...
        if (!existingUser.getPassword().equals(userDTO.getPassword())) {
            return ResponseEntity.badRequest().body(new LoginResponseDTO("Wrong password! Please try again.", null));
        }
    ...
    // "This email has only been registered by signing in through Google. To enable
    // signing in through this email, please sign up. Else try signing in with
    // Google." :
    // "The email you entered is not registered. Please make sure you've entered it
    // correctly or sign up for a new account.");
    // }
